...
class CourseNode(object):
	...
	def createNode(self, soupString): #soupNode should be a navigatable string
		...
		filter(lambda x: x in string.printable, ourseblocktitle)
		courseblocktitle = courseblocktitle.p.string.encode('utf-8').strip().split();
		#print courseblocktitle.p.string.encode('utf-8');
		...
		self.description = soupString.find('div',class_=courseblockdesc.string);
